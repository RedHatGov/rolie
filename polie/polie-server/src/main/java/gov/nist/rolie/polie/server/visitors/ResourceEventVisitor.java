...
import gov.nist.rolie.polie.atomLogic.modelServices.MismatchedCategoriesException;
...
public class ResourceEventVisitor implements RESTEventVisitor { // TODO:
	...
	public boolean visit(Post post, ResponseBuilder rb, Map<String, Object> data) {
		...
		try {
			...
			rb.status(Status.NOT_FOUND);
			rb.entity("Resource Not Found");
			return false;
		} catch (InvalidResourceTypeException e) {
			rb.status(Status.METHOD_NOT_ALLOWED);
			rb.entity("Resource is not feed, and/or cannot be POSTed to.");
			return false;
		}

		AtomFeed retrievedFeed = feed;
		...
		try {
			...
			rb.entity("This entry has invalid categories for this fixed category feed.");
			return false;
		}
		// Save the entry document to the database, we save the entry to
		// location generated by its title.
		URI entryLocation = null;
		String feedURI = feedService.searchFeedLinksForRel(retrievedFeed, "self");
		String entryTitle = entry.getXmlObject().getEntry().getTitleList().get(0);
		entryTitle = entryTitle.replace(' ', '_');
		entryTitle = entryTitle.replaceAll("[^A-Za-z0-9]", "");
		try {
			entryLocation = new URI(feedURI + "/" + entryTitle);
			entry = entryService.createEntry(entry, entryLocation);
		} catch (ResourceAlreadyExistsException e1) {
			rb.status(Status.CONFLICT);
			rb.entity("Resource already exists. ERR#1234");
			return false;
		} catch (URISyntaxException e) {
			rb.status(Status.INTERNAL_SERVER_ERROR);
			rb.entity("Error generating URI from title.");
			return false;
		}

		try {
			feedService.updateFeed(retrievedFeed, iri);
		} catch (ResourceNotFoundException e) {
			rb.status(Status.NOT_FOUND);
			rb.entity("Resource Not Found");
			return false;
		...
		rb = rb.header("Location", entryLocation.toString());
